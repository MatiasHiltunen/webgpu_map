...
fn fsMain(in : VSOut) -> @location(0) vec4<f32> {
  return vec4<f32>(textureSample(tileTexture, tileSampler, in.uv).rgb, 1.0);
}
`;
...
fn hueRotate(color : vec3<f32>, hueCos : f32, hueSin : f32) -> vec3<f32> {
...
fn fsMain(in : VSOut) -> @location(0) vec4<f32> {
  ...
  color = hueRotate(color, basemap.extra.z, basemap.extra.w);
